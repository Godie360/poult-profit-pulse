...
export class DashboardController {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Only workers can create daily logs' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.WORKER)
  async createDailyLog(@Body() createDailyLogDto: CreateDailyLogDto, @Request() req): Promise<DailyLog> {
    return this.dashboardService.createDailyLog(createDailyLogDto, req.user.id);
  }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @UseGuards(RolesGuard)
  @Roles(UserRole.WORKER, UserRole.FARMER)
  async getUserDailyLogs(@Request() req): Promise<DailyLog[]> {
    return this.dashboardService.getUserDailyLogs(req.user.id);
  }
}
